// Finger Kinematics & Haptic Activation Feedback Mapping Engine
// Maps sign language characters (letters A-Z, digits 0-9, and common words)
// to precise 5-finger anatomical activation states, flexion values, and haptic vibration patterns.

export type FingerKey = 'thumb' | 'index' | 'middle' | 'ring' | 'pinky';

export interface FingerState {
  flexion: number; // 0.0 (curled) to 1.0 (fully extended/straight)
  isActive: boolean; // Is this finger actively emphasized in the sign posture
  isLifted: boolean; // Does this finger lift/elevate in this gesture
  role: 'primary' | 'secondary' | 'anchor' | 'curled' | 'touching';
}

export interface SignFingerProfile {
  name: string;
  category: 'alphabet' | 'number' | 'word';
  primaryFingers: FingerKey[];
  activeFingers: FingerKey[];
  fingers: Record<FingerKey, FingerState>;
  description: string;
  movementType: 'static' | 'lift' | 'pulse' | 'wave' | 'pinch' | 'hook' | 'circle';
  hapticPattern: number[]; // Vibration pattern in ms, e.g. [30, 20, 15]
  soundPitch: number; // Web Audio oscillator frequency in Hz
}

// Color and styling config for the 5 fingers
export const FINGER_METADATA: Record<FingerKey, {
  name: string;
  icon: string;
  color: string;
  glowColor: string;
...
// Finger configurations for all 26 ASL alphabet letters
const ALPHABET_PROFILES: Record<string, Partial<SignFingerProfile>> = {
  'A': {
  ...
  'B': {
    ...
    fingers: {
      ...
      index: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
      middle: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
      ring: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
      pinky: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' }
    },
    soundPitch: 440,
  ...
  'L': {
    ...
    activeFingers: ['thumb', 'index'],
    movementType: 'lift',
    description: 'Thumb and index finger open at a crisp 90° right angle forming the letter L.',
    fingers: {
      thumb: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
      index: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
      middle: { flexion: 0.1, isActive: false, isLifted: false, role: 'curled' },
      ring: { flexion: 0.1, isActive: false, isLifted: false, role: 'curled' },
      pinky: { flexion: 0.1, isActive: false, isLifted: false, role: 'curled' }
...
// Numbers 0 to 9
const NUMBER_PROFILES: Record<string, Partial<SignFingerProfile>> = {
  '0': {
    primaryFingers: ['thumb', 'index', 'middle', 'ring', 'pinky'],
    activeFingers: ['thumb', 'index', 'middle', 'ring', 'pinky'],
    movementType: 'pinch',
...
// Word profiles
const WORD_PROFILES: Record<string, Partial<SignFingerProfile>> = {
  'Hello': {
    primaryFingers: ['thumb', 'index', 'middle', 'ring', 'pinky'],
    activeFingers: ['thumb', 'index', 'middle', 'ring', 'pinky'],
    movementType: 'wave',
...
/**
 * Get comprehensive finger articulation profile for any sign key name
 */
export function getFingerProfileForSign(signName: string): SignFingerProfile {
  ...
  // Default fallback for any unlisted word/sign
  return {
    name: signName,
    category: 'alphabet',
    primaryFingers: ['index'],
...
function getDefaultFingers(): Record<FingerKey, FingerState> {
  return {
    thumb: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
    index: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
    middle: { flexion: 1.0, isActive: true, isLifted: true, role: 'primary' },
...
/**
 * Triggers hardware haptic vibration feedback via Web Vibration API (mobile/touch)
 * and subtle tactile acoustic synthesizer feedback via Web Audio API (desktop/laptop)
 */
export function triggerHapticFeedback(pattern: number[] = [25], soundFreq: number = 440, soundEnabled: boolean = true) {
  // 1. Hardware Haptic Motor Vibration
  try {
  ...
  // 2. Synthesized Tactile Acoustic Click
