...
import { TFJSTelemetry, TF_SIGN_CLASSES } from '../utils/tfjsModel';

interface TensorFlowEngineHUDProps {
  telemetry?: TFJSTelemetry;
  isEnabled: boolean;
  onToggleEnabled: (enabled: boolean) => void;
  onTrainSample: (label: string) => Promise<{ success: boolean; epochs: number; loss: number }>;
  onSwitchBackend: (backend: 'webgl' | 'cpu') => Promise<void>;
}

export const TensorFlowEngineHUD: React.FC<TensorFlowEngineHUDProps> = ({
  telemetry,
  isEnabled,
  onToggleEnabled,
  onTrainSample,
  ...
  const [selectedTrainingLabel, setSelectedTrainingLabel] = useState<string>('HELLO');
  const [isTraining, setIsTraining] = useState<boolean>(false);
  const [trainingFeedback, setTrainingFeedback] = useState<string | null>(null);
  const [isSwitchingBackend, setIsSwitchingBackend] = useState<boolean>(false);
  ...
  const handleBackendToggle = async (targetBackend: 'webgl' | 'cpu') => {
  ...
  return (
    <div className="bg-slate-900/95 dark:bg-slate-950 text-white rounded-3xl border border-slate-700/80 shadow-2xl overflow-hidden transition-all">
      {/* Top Banner Bar */}
        ...
        <div className="p-5 space-y-5 bg-slate-900/50">
          ...
          <div>
            ...
            {predictions.length > 0 ? (
                      ...
                      <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-150 ${
                            isTop ? 'bg-gradient-to-r from-amber-400 to-orange-500' : 'bg-slate-600'
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
              ...
              <p className="text-xs text-slate-500 italic">Evaluating hand coordinates...</p>
            )}
          </div>
          ...
          <div className="pt-4 border-t border-slate-800">
            ...
            {trainingFeedback && (
              <div className="mt-3 px-3.5 py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-mono flex items-center space-x-2 animate-in fade-in">
