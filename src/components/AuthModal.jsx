...
const AuthModal = ({
  ...
  const [primaryLang, setPrimaryLang] = useState(currentUser.primaryLanguage);
  ...
  const handleSubmit = (e) => {
    ...
    onUpdateUser({
      ...
      certifications: role === "interpreter" ? [certification, "NIC Master"] : void 0,
      verified: true
    });
    onClose();
  };
  ...
  >
    ...
    /* Brand Icon & Heading */
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            {authMode === "signin" ? "Access real-time sign language translation & live interpreter calls" : "Join the community of signers, learners, and certified interpreters"}
          </p>
        </div>
    ...
    /* Social Logins */
        ...
        <div className="grid grid-cols-3 gap-2 mb-5">
          {["Google", "Apple", "Microsoft"].map((prov) => <button
    key={prov}
    type="button"
  ...
  >
    ...
    className={`p-2.5 rounded-xl border text-center transition-all ${role === "user_deaf" ? "bg-indigo-50 dark:bg-indigo-950/60 border-indigo-600 dark:border-indigo-400 text-indigo-600 dark:text-indigo-300 font-bold" : "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-xs font-semibold"}`}
  >
    ...
    className={`p-2.5 rounded-xl border text-center transition-all ${role === "user_hearing" ? "bg-indigo-50 dark:bg-indigo-950/60 border-indigo-600 dark:border-indigo-400 text-indigo-600 dark:text-indigo-300 font-bold" : "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-xs font-semibold"}`}
  >
    ...
    className={`p-2.5 rounded-xl border text-center transition-all ${role === "interpreter" ? "bg-purple-50 dark:bg-purple-950/60 border-purple-600 dark:border-purple-400 text-purple-600 dark:text-purple-300 font-bold" : "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-xs font-semibold"}`}
  >
  ...
  />
          ...
          {role === "interpreter" && <div className="p-3 bg-purple-50 dark:bg-purple-950/40 rounded-xl border border-purple-200 dark:border-purple-900 text-xs">
  ...
  >
              {SIGN_LANGUAGES.map((l) => <option key={l.code} value={l.code}>{l.flag} {l.name} ({l.code})</option>)}
            </select>
          </div>
  ...
  >
            {authMode === "signin" ? "Don't have an account? Sign up with tutorial options" : "Already have an account? Sign in here"}
          </button>
        </div>
