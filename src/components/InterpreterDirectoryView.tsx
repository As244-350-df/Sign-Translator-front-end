...
import { MOCK_INTERPRETERS, SIGN_LANGUAGES } from '../data/mockData';
...
export const InterpreterDirectoryView: React.FC<InterpreterDirectoryViewProps> = ({
  ...
  // Filter interpreters
  const filteredInterpreters = MOCK_INTERPRETERS.filter((int) => {
    const matchesSearch = 
      int.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      int.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      int.bio.toLowerCase().includes(searchQuery.toLowerCase()) ||
      int.specialties.some(s => s.toLowerCase().includes(searchQuery.toLowerCase()));

    const matchesLanguage = 
      selectedLanguage === 'ALL' || 
      int.languages.includes(selectedLanguage as SignLanguageCode);

    const matchesSpecialty = 
      selectedSpecialty === 'ALL' || 
      int.specialties.includes(selectedSpecialty);

    const matchesOnline = 
      !onlineOnly || int.availableStatus === 'online';

    return matchesSearch && matchesLanguage && matchesSpecialty && matchesOnline;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 p-6 sm:p-8 rounded-3xl text-white shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center gap-3">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              placeholder="Search by interpreter name, specialty (e.g. Medical, Legal), or skills..."
