...
import { getFingerProfileForSign, FingerKey, SignFingerProfile } from '../utils/fingerMapping';
import { speakText } from '../utils/speech';

export type AvatarModel = 'maya' | 'nova' | 'kai' | 'skeletal';
export type AvatarCameraAngle = 'front' | 'angled' | 'hands_zoom';
...
interface SignLanguageAvatarProps {
  ...
  onNextWord: () => void;
  onPrevWord: () => void;
  onSelectWordIndex: (idx: number) => void;
...
// Spatial 2D joint configuration for avatar upper body
interface AvatarPose {
  head: { x: number; y: number; tiltDeg: number };
  eyebrows: 'neutral' | 'raised' | 'furrowed' | 'empathetic';
  eyes: 'open' | 'blink' | 'squint' | 'wide';
  mouth: 'neutral' | 'smile' | 'open_ah' | 'round_oh' | 'fist_firm';
  ...
  leftFingers?: Partial<Record<FingerKey, number>>;
  rightFingers?: Partial<Record<FingerKey, number>>;
  gloss: string;
  category: string;
...
export const SignLanguageAvatar: React.FC<SignLanguageAvatarProps> = ({
  ...
  // Avatar Personality & Visual Mode
  const [avatarModel, setAvatarModel] = useState<AvatarModel>('maya');
  const [cameraAngle, setCameraAngle] = useState<AvatarCameraAngle>('front');
  const [showSkeletalOverlay, setShowSkeletalOverlay] = useState<boolean>(false);
  const [showSignInfoDrawer, setShowSignInfoDrawer] = useState<boolean>(true);
  const [isAudioSyncEnabled, setIsAudioSyncEnabled] = useState<boolean>(true);
  const [isFullscreen, setIsFullscreen] = useState<boolean>(false);

  // Dynamic Eye Blinking & Micro-motion Idle Cycle
  ...
  // Audio voice-over synchronization when word advances and audio sync is active
  useEffect(() => {
  ...
  // Derive active Sign Finger Profile and Motion Pose from the current sign word
  const activePose: AvatarPose = useMemo(() => {
    const rawWord = currentWord.toUpperCase().trim();
    const cleanWord = rawWord.replace(/[^A-Z0-9]/g, '');

    // 1. SPECIFIC ASL HIGH-FREQUENCY COMMON WORDS
    if (cleanWord === 'HELLO' || cleanWord === 'HI') {
      return {
        ...
        rightFingers: { thumb: 0.8, index: 1.0, middle: 1.0, ring: 1.0, pinky: 1.0 },
    ...
    if (cleanWord === 'THANK' || cleanWord === 'THANKS' || cleanWord === 'THANKYOU') {
      return {
        ...
        rightFingers: { thumb: 0.85, index: 1.0, middle: 1.0, ring: 1.0, pinky: 1.0 },
        gloss: 'THANK-YOU',
        category: 'Courtesy',
    ...
    if (cleanWord === 'PLEASE') {
      return {
        ...
        rightFingers: { thumb: 0.9, index: 1.0, middle: 1.0, ring: 1.0, pinky: 1.0 },
    ...
    if (cleanWord === 'HELP') {
      return {
        ...
        leftFingers: { thumb: 0.9, index: 1.0, middle: 1.0, ring: 1.0, pinky: 1.0 },
        rightFingers: { thumb: 1.0, index: 0.1, middle: 0.1, ring: 0.1, pinky: 0.1 },
    ...
    if (cleanWord === 'LOVE' || cleanWord === 'ILOVEYOU' || cleanWord === 'ILY') {
      return {
        ...
        rightFingers: { thumb: 1.0, index: 1.0, middle: 0.1, ring: 0.1, pinky: 1.0 },
    ...
    if (cleanWord === 'PEACE') {
      return {
        ...
        leftFingers: { thumb: 0.2, index: 1.0, middle: 1.0, ring: 0.1, pinky: 0.1 },
        rightFingers: { thumb: 0.2, index: 1.0, middle: 1.0, ring: 0.1, pinky: 0.1 },
        gloss: 'PEACE (V-Sign)',
        category: 'Symbolic',
    ...
    // 2. FALLBACK TO FINGER SPELLING OR GENERAL SIGN KINEMATICS
    const profile: SignFingerProfile = getFingerProfileForSign(cleanWord);
    const isSingleChar = cleanWord.length === 1;

    const rFingers: Record<FingerKey, number> = {
      thumb: profile.fingers.thumb?.flexion ?? 0.8,
      index: profile.fingers.index?.flexion ?? 1.0,
      middle: profile.fingers.middle?.flexion ?? 1.0,
      ring: profile.fingers.ring?.flexion ?? 1.0,
      pinky: profile.fingers.pinky?.flexion ?? 1.0
    };
  ...
  // Color & Aesthetic Palette by Avatar Model
  ...
  // Transform matrix for Camera Angles
  ...
  // Helper to render an articulated hand with 5 realistic fingers
  const renderHand = (
    wristX: number,
    wristY: number,
    rotation: number,
    scale: number,
    fingers: Partial<Record<FingerKey, number>> = {},
    isRight: boolean = true
  ) => {
    const thumbFlex = fingers.thumb ?? 0.8;
    const indexFlex = fingers.index ?? 1.0;
    const middleFlex = fingers.middle ?? 1.0;
    const ringFlex = fingers.ring ?? 1.0;
    const pinkyFlex = fingers.pinky ?? 1.0;

    // Finger length scaling by flexion (1.0 = straight, 0.2 = curled)
    const getFingerPoints = (baseX: number, baseY: number, angleDeg: number, maxLen: number, flex: number) => {
      ...
      <g transform={`translate(${wristX}, ${wristY}) rotate(${rotation}) scale(${scale})`}>
        ...
        {fingerList.map((f, idx) => (
          <g key={idx}>
            ...
            />
            {/* Knuckle Joint Shade */}
            <circle cx={f.midX} cy={f.midY} r={idx === 0 ? 2.5 : 2.0} fill={avatarTheme.skinShadow} opacity="0.7" />
        ...
        {/* Wrist Base Landmark */}
        {showSkeletalOverlay && (
          <circle cx="0" cy="10" r="3.5" fill="#10B981" stroke="#FFFFFF" strokeWidth="1" />
        )}
      </g>
    );
  };
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 mt-1 w-44 bg-slate-900 border border-slate-700 rounded-2xl shadow-xl p-1 hidden group-hover:block z-30 animate-in fade-in">
              ...
              {[
                ...
                <button
                  key={m.id}
                  onClick={() => setAvatarModel(m.id as AvatarModel)}
          ...
          >
            ...
            <g className="transition-all duration-300">
              ...
              {renderHand(
                ...
                activePose.leftFingers || { thumb: 0.8, index: 1.0, middle: 1.0, ring: 1.0, pinky: 1.0 },
                false
              )}
            </g>
            ...
            <g className="transition-all duration-300">
              ...
              {renderHand(
                ...
                activePose.rightFingers || { thumb: 0.8, index: 1.0, middle: 1.0, ring: 1.0, pinky: 1.0 },
                true
              )}
            </g>
          </svg>
