...
const InterpreterDirectoryView = ({
  ...
  >
            ...
            {SIGN_LANGUAGES.map((l) => <option key={l.code} value={l.code}>{l.flag} {l.code} - {l.name.split(" ")[0]}</option>)}
          </select>
    ...
    className="w-full md:w-auto bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500"
  >
            {specialtiesList.map((spec) => <option key={spec} value={spec}>{spec === "ALL" ? "All Specialties" : spec}</option>)}
          </select>
    ...
    className={`px-3 py-2.5 rounded-xl text-xs font-bold transition-colors whitespace-nowrap flex items-center space-x-1.5 ${onlineOnly ? "bg-emerald-500 text-white shadow-xs" : "bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-200"}`}
  >
  ...
  >
    ...
    />
                        <span className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full ring-2 ring-white dark:ring-slate-800 ${isOnline ? "bg-emerald-500" : "bg-slate-400"}`} />
                      </div>
                      <div>
                        <div className="flex items-center space-x-1">
                          ...
                          {interpreter.verified && <ShieldCheck className="w-4 h-4 text-indigo-500 shrink-0" />}
                        </div>
      ...
      /* Languages Badges */
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-4">
                    {interpreter.languages.map((lang) => <span
      key={lang}
    ...
    >
                    ...
                    {interpreter.spokenLanguages.map((spk) => <span
      key={spk}
    ...
    >
      ...
      /* Specialties Tags */
                  ...
                  <div className="mt-3 flex flex-wrap gap-1">
                    {interpreter.specialties.slice(0, 2).map((spec, i) => <span
      key={i}
