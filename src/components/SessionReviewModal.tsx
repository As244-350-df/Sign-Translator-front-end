...
export const SessionReviewModal: React.FC<SessionReviewModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'transcript' | 'clinical_summary' | 'billing'>('transcript');
            ...
            <div className="flex items-center space-x-1 text-amber-400">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Star
                    className={`w-5 h-5 ${
                      star <= userRating ? 'fill-current text-amber-400' : 'text-slate-300 dark:text-slate-700'
                    }`}
                  />
                </button>
              ))}
