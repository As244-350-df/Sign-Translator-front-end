...
import { X, Plus, Check, HandMetal } from "lucide-react";
const PRESET_EMOJIS = ["\u{1F44B}", "\u2615", "\u{1F3E0}", "\u{1F431}", "\u{1F436}", "\u{1F697}", "\u2708\uFE0F", "\u{1F355}", "\u{1F4A1}", "\u{1F511}", "\u{1F31F}", "\u{1F4F1}", "\u{1F393}", "\u{1F3E5}", "\u2764\uFE0F", "\u{1F389}", "\u{1F4BC}", "\u{1F6D1}", "\u23F0", "\u26A1"];
const PRESET_HAND_SHAPES = [
  { name: "Open Palm (5)", icon: "\u{1F590}\uFE0F", config: { thumb: 1, index: 1, middle: 1, ring: 1, pinky: 1 } },
  { name: "Solid Fist (S/A)", icon: "\u270A", config: { thumb: 0.2, index: 0.2, middle: 0.2, ring: 0.2, pinky: 0.2 } },
  { name: "Point Index (D/1)", icon: "\u261D\uFE0F", config: { thumb: 0.3, index: 1, middle: 0.2, ring: 0.2, pinky: 0.2 } },
  { name: "V-Shape / Peace (V/2)", icon: "\u270C\uFE0F", config: { thumb: 0.3, index: 1, middle: 1, ring: 0.2, pinky: 0.2 } },
  { name: "Thumbs Up (10)", icon: "\u{1F44D}", config: { thumb: 1, index: 0.2, middle: 0.2, ring: 0.2, pinky: 0.2 } },
  { name: "I Love You (ILY)", icon: "\u{1F91F}", config: { thumb: 1, index: 1, middle: 0.2, ring: 0.2, pinky: 1 } },
  { name: "Call Me / Shaka (Y)", icon: "\u{1F919}", config: { thumb: 1, index: 0.2, middle: 0.2, ring: 0.2, pinky: 1 } },
  { name: "OK / Pinch (F/9)", icon: "\u{1F44C}", config: { thumb: 0.5, index: 0.5, middle: 1, ring: 1, pinky: 1 } },
  { name: "Three / Water (W/3)", icon: "\u{1F4A7}", config: { thumb: 0.3, index: 1, middle: 1, ring: 1, pinky: 0.2 } },
  { name: "Four (4)", icon: "4\uFE0F\u20E3", config: { thumb: 0.2, index: 1, middle: 1, ring: 1, pinky: 1 } }
];
const AddSignModal = ({ onClose, onSaveSign }) => {
  const [signName, setSignName] = useState("");
  const [translatedText, setTranslatedText] = useState("");
  const [symbol, setSymbol] = useState("\u{1F31F}");
  const [meaning, setMeaning] = useState("");
  const [category, setCategory] = useState("custom");
  const [aslNotation, setAslNotation] = useState("");
  const [selectedShapePreset, setSelectedShapePreset] = useState("Open Palm (5)");
  ...
  const handleSelectShapePreset = (preset) => {
    setSelectedShapePreset(preset.name);
    setFingerConfig(preset.config);
  };
  const handleFingerSliderChange = (finger, val) => {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in overflow-y-auto">
  ...
  />
                <div className="flex flex-wrap gap-1 overflow-x-auto max-h-12 py-1">
                  {PRESET_EMOJIS.slice(0, 10).map((em) => <button
    key={em}
    type="button"
    onClick={() => setSymbol(em)}
    className={`w-7 h-7 rounded-lg text-sm flex items-center justify-center transition-transform hover:scale-110 ${symbol === em ? "bg-indigo-600 text-white" : "bg-slate-100 dark:bg-slate-800"}`}
  >
                      {em}
                    </button>)}
                </div>
  ...
  />
    ...
    /* Quick Shape Chips */
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
              {PRESET_HAND_SHAPES.map((shape) => <button
    key={shape.name}
    type="button"
    onClick={() => handleSelectShapePreset(shape)}
    className={`p-2 rounded-xl border text-left flex items-center space-x-2 transition-all ${selectedShapePreset === shape.name ? "bg-indigo-600 text-white border-indigo-600 shadow-xs" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-indigo-300"}`}
  >
    ...
    /* Finger Extension Sliders */
            ...
            <div className="pt-2 grid grid-cols-1 sm:grid-cols-5 gap-3">
              {["thumb", "index", "middle", "ring", "pinky"].map((finger) => <div key={finger} className="space-y-1">
