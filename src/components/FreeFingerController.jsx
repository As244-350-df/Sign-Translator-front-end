...
const FreeFingerController = ({
  ...
  const [physicsTelemetry, setPhysicsTelemetry] = useState(handTracker.getPhysicsTelemetry());
  ...
  const fingersConfig = [
    { key: "thumb", label: "Thumb", icon: "\u{1F590}\uFE0F", color: "from-amber-500 to-orange-500" },
    { key: "index", label: "Index", icon: "\u261D\uFE0F", color: "from-cyan-500 to-blue-500" },
    { key: "middle", label: "Middle", icon: "\u{1F595}", color: "from-indigo-500 to-purple-500" },
    { key: "ring", label: "Ring", icon: "\u{1F48D}", color: "from-purple-500 to-pink-500" },
    { key: "pinky", label: "Pinky", icon: "\u{1F919}", color: "from-rose-500 to-red-500" }
  ];
  return <div className={`bg-slate-900/95 backdrop-blur-md border border-slate-700/80 rounded-3xl p-4 sm:p-5 shadow-2xl text-white ${className}`}>
    ...
    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 ${pose.isFreeMotion ? "bg-emerald-600 text-white shadow-md shadow-emerald-500/25" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  ...
  >
    ...
    className={`p-2 rounded-xl text-left border transition-all ${physicsConfig.preset === "biological" ? "bg-emerald-950/60 border-emerald-500 text-emerald-200 shadow-sm" : "bg-slate-800/60 border-slate-700/60 text-slate-300 hover:bg-slate-800"}`}
  >
    ...
    className={`p-2 rounded-xl text-left border transition-all ${physicsConfig.preset === "snappy" ? "bg-cyan-950/60 border-cyan-500 text-cyan-200 shadow-sm" : "bg-slate-800/60 border-slate-700/60 text-slate-300 hover:bg-slate-800"}`}
  >
            <div className="text-xs font-bold flex items-center space-x-1">
              <span>⚡</span>
    ...
    className={`p-2 rounded-xl text-left border transition-all ${physicsConfig.preset === "fluid" ? "bg-indigo-950/60 border-indigo-500 text-indigo-200 shadow-sm" : "bg-slate-800/60 border-slate-700/60 text-slate-300 hover:bg-slate-800"}`}
  >
            <div className="text-xs font-bold flex items-center space-x-1">
              <span>🌊</span>
    ...
    className={`p-2 rounded-xl text-left border transition-all ${physicsConfig.preset === "precision" ? "bg-purple-950/60 border-purple-500 text-purple-200 shadow-sm" : "bg-slate-800/60 border-slate-700/60 text-slate-300 hover:bg-slate-800"}`}
  >
            <div className="text-xs font-bold flex items-center space-x-1">
              <span>🦾</span>
  ...
  >
    ...
    /* Advanced Physics Tuning Sliders */
        ...
        {showPhysicsAdvanced && <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mt-3 pt-3 border-t border-slate-800">
            {
    /* Stiffness */
  }
  ...
  >
    ...
    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 ${activePreset === "wave" ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md" : "bg-slate-800/80 text-slate-300 hover:bg-slate-700"}`}
  >
            <span>🌊</span>
    ...
    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 ${activePreset === "wiggle" ? "bg-gradient-to-r from-purple-500 to-pink-600 text-white shadow-md" : "bg-slate-800/80 text-slate-300 hover:bg-slate-700"}`}
  >
            <span>✨</span>
    ...
    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 ${activePreset === "tap" ? "bg-gradient-to-r from-amber-500 to-orange-600 text-white shadow-md" : "bg-slate-800/80 text-slate-300 hover:bg-slate-700"}`}
  >
            <span>🎹</span>
            <span>Piano Tap</span>
          </button>
    ...
    /* Quick Static Posture Presets with Tight Anatomical Calibration */
          ...
          <button
    onClick={() => applyPreset("open", { thumb: 1, index: 1, middle: 1, ring: 1, pinky: 1, spread: 0.5, tension: 0.9 })}
    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activePreset === "open" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  >
            🖐️ Open (50% Spread)
          </button>

          <button
    onClick={() => applyPreset("fist", { thumb: 0.05, index: 0.05, middle: 0.05, ring: 0.05, pinky: 0.05, spread: 0.15, tension: 1 })}
    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activePreset === "fist" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  >
            ✊ Tight Fist
          </button>

          <button
    onClick={() => applyPreset("peace", { thumb: 0.1, index: 1, middle: 1, ring: 0.05, pinky: 0.05, spread: 0.45, tension: 0.95 })}
    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activePreset === "peace" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  >
            ✌️ Crisp Peace (V)
          </button>

          <button
    onClick={() => applyPreset("ily", { thumb: 1, index: 1, middle: 0.05, ring: 0.05, pinky: 1, spread: 0.65, tension: 0.95 })}
    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activePreset === "ily" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  >
            🤟 Love (ILY)
          </button>

          <button
    onClick={() => applyPreset("point", { thumb: 0.15, index: 1, middle: 0.05, ring: 0.05, pinky: 0.05, spread: 0.3, tension: 0.95 })}
    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activePreset === "point" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  >
            ☝️ Crisp Point / 1
          </button>

          <button
    onClick={() => applyPreset("ok", { thumb: 0.35, index: 0.35, middle: 1, ring: 1, pinky: 1, spread: 0.5, tension: 0.95 })}
    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activePreset === "ok" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  >
            👌 OK / ASL F
          </button>

          <button
    onClick={() => applyPreset("shaka", { thumb: 1, index: 0.05, middle: 0.05, ring: 0.05, pinky: 1, spread: 0.7, tension: 0.95 })}
    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activePreset === "shaka" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}
  >
            🤙 Shaka / ASL Y
          </button>
        </div>
    ...
    return <div key={f.key} className="bg-slate-800/80 p-3 rounded-2xl border border-slate-700/60 flex flex-col space-y-2">
                <div className="flex items-center justify-between">
