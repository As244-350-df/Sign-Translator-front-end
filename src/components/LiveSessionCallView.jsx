...
const LiveSessionCallView = ({
  ...
  const [recognizedSignLogs, setRecognizedSignLogs] = useState([
    ...
    { symbol: "\u{1F64F}", text: "Thank you", confidence: 0.96, time: "10:03:05" }
  ]);
      ...
      const acquireStream = async () => {
        ...
        try {
          s = await navigator.mediaDevices.getUserMedia({
            video: { facingMode: settings.cameraFacing ? { ideal: settings.cameraFacing } : void 0, width: { ideal: 1280 }, height: { ideal: 720 } },
            audio: true
          });
        } catch {
          try {
  ...
  const filteredSigns = selectedCategory === "all" ? dictionaryList : dictionaryList.filter((s) => s.category === selectedCategory);
  return <div className="relative min-h-[calc(100vh-8rem)] flex flex-col bg-slate-950 text-white rounded-3xl overflow-hidden shadow-2xl border border-slate-800">
  ...
  />
    ...
    /* Recording Badge */
          ...
          {isRecording && <div className="flex items-center space-x-1.5 px-3 py-1 rounded-full bg-rose-950/90 border border-rose-700 text-xs font-mono font-bold text-rose-300 animate-pulse">
    ...
    className={`p-2 rounded-xl text-xs font-bold transition-all flex items-center space-x-1 border ${showDiagnostics ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-900/80 border-slate-700/60 text-slate-300 hover:bg-slate-800"}`}
  ...
  >
    ...
    /* Remote Live Interpreter Video Feed OR Main Real Camera */
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          {mainViewMode === "camera" && useRealCameraLocal ? <video
    ref={mainVideoRef}
    autoPlay
    muted
    playsInline
  ...
  />}
    ...
    width={1280}
    height={720}
    className={`absolute inset-0 w-full h-full pointer-events-none transition-opacity duration-200 ${showLandmarkOverlay ? "opacity-100" : "opacity-0"}`}
  />

          {
    /* Committed Sign Live Notification Banner */
  }
          {lastCommittedBanner && <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-30 flex items-center space-x-3 px-5 py-2.5 rounded-2xl bg-emerald-950/90 backdrop-blur-md border border-emerald-500/80 shadow-2xl text-emerald-200 animate-in slide-in-from-top-4 fade-in duration-300">
  ...
  >
    ...
    /* Dialect Sign Speed Indicator */
          ...
          {signSpeed !== 1 && <div className="absolute top-20 right-6 px-3 py-1 rounded-full bg-slate-900/80 backdrop-blur-md border border-slate-700 text-xs font-bold text-indigo-400">
  ...
  >
    ...
    className={`p-1.5 rounded-lg border border-slate-700 text-slate-300 hover:text-white transition-colors ${captionSpeaking ? "bg-indigo-600 text-white animate-pulse" : "bg-slate-900"}`}
  ...
  >
              ...
              <p className={`font-semibold tracking-wide text-slate-100 ${settings.fontSize === "extra-large" ? "text-xl" : settings.fontSize === "large" ? "text-lg" : "text-sm sm:text-base"}`}>
                {currentCaption}
              </p>
            </div>
          </div>
    ...
    /* Local User Self-View PiP */
          ...
          <div className="absolute bottom-28 right-6 w-36 sm:w-48 h-28 sm:h-36 rounded-2xl bg-slate-950 border-2 border-slate-700 shadow-2xl overflow-hidden z-20 group">
            {isCameraOff ? <div className="w-full h-full flex flex-col items-center justify-center bg-slate-900 text-slate-400">
                <CameraOff className="w-6 h-6 mb-1" />
                <span className="text-[10px]">Camera Off</span>
              </div> : useRealCameraLocal ? <video
    ref={localVideoRef}
    autoPlay
    muted
    playsInline
  ...
  >
    ...
    /* WebRTC SFU Live Telemetry Drawer */
          ...
          {showDiagnostics && <div className="absolute top-20 left-6 z-30 w-72 bg-slate-950/90 backdrop-blur-md border border-slate-700 rounded-2xl p-4 text-xs font-mono shadow-2xl space-y-2.5 animate-in fade-in">
    ...
    /* In-Call Hand Sign Recognition Deck Drawer */
        ...
        {showSignDeck && <div className="absolute left-0 top-0 bottom-24 w-80 sm:w-96 bg-slate-900/95 backdrop-blur-md border-r border-slate-800 z-30 flex flex-col p-4 shadow-2xl animate-in slide-in-from-left duration-200">
  ...
  >
    ...
    className={`py-1.5 rounded-lg font-bold transition-all flex items-center justify-center space-x-1.5 ${deckTab === "signs" ? "bg-indigo-600 text-white shadow-xs" : "text-slate-400 hover:text-white"}`}
  >
                <BookOpen className="w-3.5 h-3.5" />
    ...
    className={`py-1.5 rounded-lg font-bold transition-all flex items-center justify-center space-x-1.5 ${deckTab === "free_fingers" ? "bg-emerald-600 text-white shadow-xs" : "text-slate-400 hover:text-white"}`}
  >
  ...
  />
    ...
    /* Category Filter Pills */
                ...
                <div className="flex items-center space-x-1.5 overflow-x-auto py-2 border-b border-slate-800 text-[10px] font-bold">
                  {["all", "custom", "greetings", "emergency", "common", "actions", "numbers", "alphabet"].map((cat) => <button
    key={cat}
    onClick={() => setSelectedCategory(cat)}
    className={`px-2.5 py-1 rounded-lg uppercase whitespace-nowrap transition-colors ${selectedCategory === cat ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-400 hover:text-white"}`}
  >
                      {cat}
                    </button>)}
                </div>
    ...
    return <div
      ...
      className={`p-3 rounded-2xl border transition-all cursor-pointer flex items-center justify-between ${isActive ? "bg-indigo-950/60 border-indigo-500 shadow-md ring-1 ring-indigo-500" : "bg-slate-800/60 border-slate-700/60 hover:bg-slate-800"}`}
    >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="font-bold text-white text-xs flex items-center space-x-1.5">
                              ...
                              {sign.isCustom && <span className="px-1 py-0.2 rounded bg-purple-900 text-purple-300 text-[9px]">Custom</span>}
                            </div>
    ...
    /* Recognized History Log */
            ...
            <div className="pt-2 border-t border-slate-800 text-[11px]">
              ...
              <div className="flex space-x-1.5 overflow-x-auto pb-1">
                {recognizedSignLogs.slice(-5).map((log, i) => <div key={i} className="px-2 py-1 rounded-lg bg-slate-800/80 border border-slate-700 text-[10px] text-slate-200 whitespace-nowrap flex items-center space-x-1">
    ...
    /* Side In-Call Text Chat Drawer */
        ...
        {showChat && <div className="absolute right-0 top-0 bottom-24 w-80 sm:w-96 bg-slate-900/95 backdrop-blur-md border-l border-slate-800 z-30 flex flex-col p-4 shadow-2xl animate-in slide-in-from-right duration-200">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center space-x-2">
  ...
  >
    ...
    /* Messages List */
            ...
            <div className="flex-1 overflow-y-auto space-y-3 py-3 pr-1 text-xs">
              {chatMessages.map((msg, idx) => <div
    key={idx}
    className={`p-3 rounded-2xl max-w-[85%] ${msg.isSelf ? "ml-auto bg-indigo-600 text-white rounded-br-xs" : "bg-slate-800 text-slate-200 rounded-bl-xs"}`}
  >
  ...
  >
    ...
    className={`px-3 py-2 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 border ${showSignDeck ? "bg-emerald-600 border-emerald-500 text-white shadow-xs" : "bg-slate-900 border-slate-800 text-emerald-400 hover:text-white hover:bg-slate-800"}`}
  ...
  >
    ...
    className={`px-3 py-2 rounded-xl text-xs font-bold transition-colors flex items-center space-x-1.5 ${showLandmarkOverlay ? "bg-indigo-600 text-white shadow-xs" : "bg-slate-900 text-slate-400 hover:text-white border border-slate-800"}`}
  ...
  >
    ...
    className={`p-1 rounded text-[10px] font-bold flex items-center space-x-1 transition-colors ${isAutoCentering ? "bg-emerald-600 text-white shadow-xs" : "text-slate-400 hover:text-white"}`}
  ...
  >
    ...
    className={`p-1 rounded text-[10px] font-bold flex items-center space-x-0.5 ${showAlignmentGuide ? "bg-emerald-600 text-white" : "text-slate-400 hover:text-white"}`}
  ...
  >
    ...
    /* Speed Selector */
          ...
          <div className="flex items-center space-x-1 bg-slate-900 p-1 rounded-xl border border-slate-800 text-xs">
            {[0.75, 1, 1.25].map((spd) => <button
    key={spd}
    onClick={() => setSignSpeed(spd)}
    className={`px-2 py-1 rounded-lg text-[11px] font-bold transition-colors ${signSpeed === spd ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white"}`}
  >
                {spd}x
              </button>)}
          </div>
    ...
    className={`p-3.5 rounded-2xl transition-all shadow-md ${isMuted ? "bg-rose-600 text-white hover:bg-rose-700" : "bg-slate-800 hover:bg-slate-700 text-slate-200"}`}
  ...
  >
    ...
    className={`p-3.5 rounded-2xl transition-all shadow-md ${isCameraOff ? "bg-rose-600 text-white hover:bg-rose-700" : "bg-slate-800 hover:bg-slate-700 text-slate-200"}`}
  ...
  >
    ...
    className={`p-3.5 rounded-2xl transition-all shadow-md ${isHandRaised ? "bg-amber-500 text-white hover:bg-amber-600 animate-bounce" : "bg-slate-800 hover:bg-slate-700 text-slate-200"}`}
  ...
  >
    ...
    className={`p-3.5 rounded-2xl transition-all shadow-md ${isRecording ? "bg-rose-600 text-white animate-pulse" : "bg-slate-800 hover:bg-slate-700 text-slate-200"}`}
  ...
  >
    ...
    className={`p-3 rounded-2xl transition-colors border ${showChat ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800"}`}
