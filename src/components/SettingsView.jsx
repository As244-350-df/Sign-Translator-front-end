...
const SettingsView = ({
  ...
  return <div className="space-y-6 max-w-4xl mx-auto">
    ...
    return <button
      ...
      className={`p-4 rounded-2xl border-2 text-left transition-all ${isSelected ? "bg-indigo-50/80 dark:bg-indigo-950/60 border-indigo-600 dark:border-indigo-400 shadow-sm" : "bg-slate-50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 hover:border-indigo-300"}`}
    >
  ...
  />
    ...
    /* Physics Presets Grid */
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 pt-1">
          {["biological", "snappy", "fluid", "precision"].map((preset) => {
    const currentPreset = settings.handPhysics?.preset || "biological";
    const isSel = currentPreset === preset;
    const labels = {
      biological: { title: "Biological Realism", icon: "\u{1F9EC}", desc: "Anatomical tendon lag & tissue resistance" },
      snappy: { title: "Snappy Spring", icon: "\u26A1", desc: "High frequency & instant rebound" },
      fluid: "fluid" === preset ? { title: "Fluid Organic", icon: "\u{1F30A}", desc: "Viscous damping & smooth arcs" } : { title: "", icon: "", desc: "" },
      precision: { title: "Precision Studio", icon: "\u{1F9BE}", desc: "Zero lag & high stiffness" }
    };
    ...
    return <button
      ...
      className={`p-3.5 rounded-2xl border text-left transition-all ${isSel ? "bg-indigo-50/80 dark:bg-indigo-950/50 border-indigo-500 text-indigo-950 dark:text-indigo-200 shadow-xs" : "bg-slate-50 dark:bg-slate-900/60 border-slate-200/70 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700"}`}
    >
