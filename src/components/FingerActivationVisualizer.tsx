...
import { SkeletalHandVisualizer, VelocityMode } from './SkeletalHandVisualizer';

interface FingerActivationVisualizerProps {
...
export const FingerActivationVisualizer: React.FC<FingerActivationVisualizerProps> = ({
  currentSignName,
  onFingerSelect,
  selectedFingerFilter = null,
  className = ''
}) => {
  const [profile, setProfile] = useState<SignFingerProfile>(() => getFingerProfileForSign(currentSignName));
  const [hapticPulseTrigger, setHapticPulseTrigger] = useState<number>(0);
  const [hapticsEnabled, setHapticsEnabled] = useState<boolean>(true);
  const [soundEnabled, setSoundEnabled] = useState<boolean>(true);
  const [viewMode, setViewMode] = useState<'skeleton' | 'gauges' | 'split'>('skeleton');
  const [velocityMode, setVelocityMode] = useState<VelocityMode>('auto');
  ...
  // Current dynamic spring physics configuration based on velocity
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 shadow-xs transition-all ${className}`}>
        ...
        <div className="flex flex-wrap items-center gap-1.5 self-end sm:self-center">
          ...
          <button
            ...
            className={`p-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1 transition-all ${
              hapticsEnabled
                ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-300 border border-emerald-500/30'
          ...
          <button
            ...
            className={`p-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1 transition-all ${
              soundEnabled
                ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30'
                : 'bg-slate-100 dark:bg-slate-800 text-slate-400'
            }`}
      ...
      <div className="py-2">
        ...
        {(viewMode === 'skeleton' || viewMode === 'split') && (
          <div className="relative bg-slate-950/90 rounded-2xl p-3 border border-slate-800/80 shadow-inner mb-3 overflow-hidden">
        ...
        {(viewMode === 'gauges' || viewMode === 'split') && (
          <div className="grid grid-cols-5 gap-2 sm:gap-3 items-end min-h-[170px] pt-3 pb-2 px-1">
            {fingerKeys.map((fKey) => {
              ...
              // Compute visual lift height based on state
              ...
              return (
                <div 
                  ...
                  className={`relative flex flex-col items-center justify-end cursor-pointer group transition-transform ${
                    isSelectedFilter ? 'ring-2 ring-indigo-500 rounded-2xl p-1 bg-indigo-50/50 dark:bg-indigo-950/30' : ''
                  }`}
                >
                  ...
                  >
                    ...
                    <div className="relative mb-1">
                      <div 
                        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center text-sm shadow-md transition-all ${
                          isPrimary
                            ? `bg-gradient-to-tr ${meta.color} text-white ring-4 ring-indigo-500/20 scale-105`
                            : state.flexion > 0.3
                    ...
                    <div className="w-5 sm:w-6 h-16 sm:h-20 bg-slate-100 dark:bg-slate-800/80 rounded-xl p-1 flex flex-col justify-end border border-slate-200/80 dark:border-slate-700/80 overflow-hidden relative shadow-inner">
                      ...
                      <motion.div
                        ...
                        className={`w-full rounded-lg transition-colors ${
                          isPrimary
                            ? `bg-gradient-to-t ${meta.color} shadow-md`
                            : state.flexion > 0.3
      ...
      <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-1.5">
        ...
        {profile.primaryFingers.length === 0 && (
          <span className="text-xs text-slate-400 italic">No specific isolated fingers (Full fist shape)</span>
        )}
      </div>
