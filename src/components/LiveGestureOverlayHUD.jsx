...
import { GESTURE_DICTIONARY } from "../utils/gestureMapping";
  ...
  return (
    <>
      ...
      <div className="absolute top-4 left-4 flex flex-wrap items-center gap-2 z-20 pointer-events-auto">
        ...
        {/* Gesture Mapping Guide Button */}
        <button
          onClick={() => setShowGestureGuide((prev) => !prev)}
          className={`flex items-center space-x-1.5 backdrop-blur-md px-2.5 py-1.5 rounded-full border text-[11px] font-bold shadow-lg transition-all cursor-pointer ${
            showGestureGuide
              ? "bg-indigo-600 text-white border-indigo-400"
              : "bg-slate-900/90 text-slate-300 hover:text-white border-slate-700/80 hover:bg-slate-800"
          }`}
          title="Open MediaPipe Gesture Mapping Guide & Live Recognition Inspector"
        >
          <BookOpen className="w-3.5 h-3.5 text-indigo-400" />
          <span>Gesture Map ({Object.keys(GESTURE_DICTIONARY).length})</span>
        </button>
      </div>
      ...
      {/* Gesture Mapping Guide Modal */}
      {showGestureGuide && (
        <div className="absolute inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 pointer-events-auto animate-in fade-in">
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white flex items-center gap-2">
                    MediaPipe Gesture Mapping Engine
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 font-mono">
                      {Object.keys(GESTURE_DICTIONARY).length} Active Rules
                    </span>
                  </h3>
                  <p className="text-xs text-slate-400">
                    Kinematic finger flexion ratios & orientation mapped from 21 3D landmarks
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Gesture Grid */}
            <div className="p-4 overflow-y-auto space-y-2.5 divide-y divide-slate-800/60 max-h-[60vh]">
              {Object.entries(GESTURE_DICTIONARY).map(([id, g]) => {
                const isCurrent = activeSign?.signName === g.name || activeSign?.signName === id;
                return (
                  <div
                    key={id}
                    className={`pt-2.5 first:pt-0 flex items-start justify-between gap-3 rounded-xl p-2.5 transition-colors ${
                      isCurrent
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                          ...
                          {isCurrent && (
                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 font-bold animate-pulse">
                              ACTIVE LIVE
                            </span>
                          )}
                        </div>
                        <p className="text-[11px] text-slate-300 mt-0.5">{g.meaning}</p>
            ...
            <div className="p-3 bg-slate-950/60 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
              <span>MediaPipe Hands Lite • 21 Landmarks • Zero Server Latency</span>
              <button
                onClick={() => setShowGestureGuide(false)}
