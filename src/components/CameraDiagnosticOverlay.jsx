...
const CameraDiagnosticOverlay = ({
  ...
  const handleRunDirectCameraTest = async () => {
    ...
    try {
      ...
      const resText = settings?.width ? `${settings.width}\xD7${settings.height}` : "Active";
      setManualTestMessage(`Success! Camera stream acquired (${resText}, label: "${track?.label || "Webcam"}"). Your camera hardware and browser permissions are fully operational!`);
  ...
  const aspectLabel = aspectRatio === "1.78" ? "16:9 (Widescreen HD)" : aspectRatio === "1.33" ? "4:3 (Standard)" : aspectRatio === "1.00" ? "1:1 (Square)" : aspectRatio !== "0" ? `${aspectRatio}:1` : "Not Resolved";
  ...
  >
          ...
          <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col justify-between">
            ...
            <div className="mt-1">
              <p className="text-xs font-mono font-bold text-white">
                {reportedWidth > 0 && reportedHeight > 0 ? `${reportedWidth} \xD7 ${reportedHeight}` : "0 \xD7 0 (Offline)"}
              </p>
    ...
    /* Track State */
          ...
          <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col justify-between">
            ...
            <div className="mt-1 flex items-center space-x-1.5">
              <span className={`w-2 h-2 rounded-full ${trackState === "live" && !trackMuted ? "bg-emerald-400 animate-pulse" : trackMuted ? "bg-amber-400" : "bg-rose-400"}`} />
              <span className={`text-xs font-bold uppercase ${trackState === "live" && !trackMuted ? "text-emerald-400" : trackMuted ? "text-amber-400" : "text-rose-400"}`}>
    ...
    /* Light / Luminance Sensor */
          ...
          <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col justify-between">
            <span className="text-[11px] font-medium text-slate-400 flex items-center space-x-1">
              {isLuminanceDark ? <Moon className="w-3 h-3 text-amber-400" /> : <Sun className="w-3 h-3 text-amber-400" />}
              <span>Luminance</span>
            </span>
            <div className="mt-1 flex items-center space-x-1.5">
              ...
              {isLuminanceDark && <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-500/20 text-rose-300 font-bold">
                  Dark
                </span>}
            </div>
          </div>
    ...
    className={`pb-2.5 px-3 text-xs font-bold border-b-2 transition-colors cursor-pointer flex items-center space-x-1.5 shrink-0 ${activeTab === "why_not_working" ? "border-amber-500 text-amber-400" : "border-transparent text-slate-400 hover:text-slate-200"}`}
  >
    ...
    className={`pb-2.5 px-3 text-xs font-bold border-b-2 transition-colors cursor-pointer flex items-center space-x-1.5 shrink-0 ${activeTab === "metrics" ? "border-indigo-500 text-indigo-400" : "border-transparent text-slate-400 hover:text-slate-200"}`}
  >
    ...
    className={`pb-2.5 px-3 text-xs font-bold border-b-2 transition-colors cursor-pointer flex items-center space-x-1.5 shrink-0 ${activeTab === "troubleshoot" ? "border-indigo-500 text-indigo-400" : "border-transparent text-slate-400 hover:text-slate-200"}`}
  >
    ...
    className={`pb-2.5 px-3 text-xs font-bold border-b-2 transition-colors cursor-pointer flex items-center space-x-1.5 shrink-0 ${activeTab === "raw" ? "border-indigo-500 text-indigo-400" : "border-transparent text-slate-400 hover:text-slate-200"}`}
  >
    ...
    /* Content Body */
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1">
          {
    /* TAB 0: WHY WEBCAM IS NOT WORKING - COMPREHENSIVE FIX GUIDE */
  }
          {activeTab === "why_not_working" && <div className="space-y-4 text-xs">
              
              {
    /* IFRAME PREVIEW ALERT (MOST COMMON CAUSE IN AI STUDIO) */
  }
              {isInIframe && <div className="p-4 rounded-2xl bg-amber-950/70 border border-amber-500/50 shadow-lg text-amber-200 animate-in fade-in">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 space-y-1.5">
                      ...
                      <p className="text-slate-300 leading-relaxed text-[11px]">
                        Modern web browsers (Chrome, Edge, Safari, Firefox) frequently block or suppress camera access prompts when web applications run inside an embedded <code className="bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">&lt;iframe&gt;</code> sandbox.
                      </p>
                      <div className="pt-2">
                        <a
    ...
    onClick={() => {
      if (navigator?.mediaDevices?.enumerateDevices) {
        ...
        navigator.mediaDevices.enumerateDevices().then((devs) => {
          setDetectedCameras(devs.filter((d) => d.kind === "videoinput"));
          setIsCheckingDevices(false);
        });
      }
    }}
  ...
  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {
    /* Scan Item 1: Physical Cameras */
  }
                  <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-slate-400 text-[11px]">Physical Video Capture Devices</span>
                      {detectedCameras.length > 0 ? <span className="text-[11px] font-bold text-emerald-400 flex items-center space-x-1">
                          <CheckCircle2 className="w-3.5 h-3.5" />
                          <span>{detectedCameras.length} Detected</span>
                        </span> : <span className="text-[11px] font-bold text-rose-400 flex items-center space-x-1">
                          <XCircle className="w-3.5 h-3.5" />
                          <span>0 Detected</span>
                        </span>}
                    </div>
                    {detectedCameras.length > 0 ? <ul className="text-[11px] text-slate-300 font-mono space-y-0.5">
                        {detectedCameras.map((c, i) => <li key={i} className="truncate text-indigo-200">
                            • {c.label || `Video Input Device #${i + 1}`}
                          </li>)}
                      </ul> : <p className="text-[11px] text-rose-300/90 leading-tight mt-1">
                        No webcam hardware detected by OS. Check USB connection or laptop camera switch.
                      </p>}
                  </div>

                  {
    /* Scan Item 2: Browser Permission State */
  }
                  <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-slate-400 text-[11px]">Browser Permission Status</span>
                      {permissionStatus === "granted" ? <span className="text-[11px] font-bold text-emerald-400 flex items-center space-x-1">
                          <ShieldCheck className="w-3.5 h-3.5" />
                          <span>Allowed</span>
                        </span> : permissionStatus === "denied" ? <span className="text-[11px] font-bold text-rose-400 flex items-center space-x-1">
                          <ShieldAlert className="w-3.5 h-3.5" />
                          <span>Blocked</span>
                        </span> : <span className="text-[11px] font-bold text-amber-400 flex items-center space-x-1">
                          <AlertTriangle className="w-3.5 h-3.5" />
                          <span>Prompt / Waiting</span>
                        </span>}
                    </div>
                    <p className="text-[11px] text-slate-300 mt-1 leading-tight">
                      {permissionStatus === "granted" ? "Your browser has granted camera permission to this domain." : permissionStatus === "denied" ? "Camera is blocked in browser settings. See unblock instructions below." : 'Browser is waiting for user to click "Allow" in address bar prompt.'}
                    </p>
                  </div>
                </div>
              </div>
    ...
    /* DIRECT MANUAL TEST CARD */
              ...
              <div className="p-4 rounded-2xl bg-indigo-950/40 border border-indigo-500/30 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
    onClick={handleRunDirectCameraTest}
  ...
  >
                    {manualTestStatus === "testing" ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5" />}
                    <span>{manualTestStatus === "testing" ? "Testing..." : "Run Test Now"}</span>
                  </button>
                </div>

                {manualTestStatus !== "idle" && <div className={`p-3 rounded-xl border text-xs ${manualTestStatus === "success" ? "bg-emerald-950/60 border-emerald-500/40 text-emerald-200" : manualTestStatus === "error" ? "bg-rose-950/60 border-rose-500/40 text-rose-200" : "bg-indigo-950/60 border-indigo-500/40 text-indigo-200"}`}>
                    <p className="font-semibold mb-2">{manualTestMessage}</p>
                    
                    {
    /* Live mini video preview if test succeeded */
  }
                    {manualTestStatus === "success" && <div className="mt-2 w-48 aspect-video rounded-lg overflow-hidden border border-emerald-500/40 bg-black">
                        <video
    ref={manualVideoRef}
    autoPlay
    muted
  ...
  />
              ...
              {
    /* STEP-BY-STEP UNBLOCKING GUIDE FOR ALL BROWSERS */
  }
              ...
              {
    /* COMMON PHYSICAL & SOFTWARE CONFLICTS */
  }
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-2.5">
                ...
                <ul className="space-y-2 text-[11px] text-slate-300">
                  <li className="flex items-start space-x-2">
                    ...
                    <div>
                      <strong className="text-white">Physical Privacy Shutter / Slider:</strong> Many modern laptops (Lenovo ThinkPad, HP, Dell, Asus) have a tiny physical mechanical slide covering the webcam lens. Check for a red dot or physical cover above your screen.
                    </div>
                  </li>
                  ...
                  <li className="flex items-start space-x-2">
                    <span className="text-indigo-400 font-bold">•</span>
                    <div>
                      <strong className="text-white">Laptop Keyboard Hotkey / Mute Switch:</strong> Some laptops have a camera killswitch button on the keyboard (typically <code className="bg-slate-900 px-1 rounded text-cyan-300">F8</code>, <code className="bg-slate-900 px-1 rounded text-cyan-300">F10</code>, or a physical toggle on the side of the chassis).
                    </div>
                  </li>
                  ...
                  <li className="flex items-start space-x-2">
                    <span className="text-indigo-400 font-bold">•</span>
                    <div>
                      <strong className="text-white">Exclusive Camera Lock by Other Apps:</strong> Webcams can only be used by one application at a time. If <strong>Zoom, Microsoft Teams, Google Meet, OBS Studio, Discord, or Skype</strong> is running in the background, close them and click Restart Camera.
                    </div>
                  </li>
                </ul>
              </div>
    ...
    /* Pitch Dark Alert Banner if applicable */
              ...
              {isLuminanceDark && streamStatus === "active" && <div className="p-4 rounded-2xl bg-amber-950/40 border border-amber-500/40 flex items-start space-x-3 text-amber-200 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="p-3 rounded-xl bg-slate-900 border border-slate-800/80">
                    ...
                    <p className="text-[11px] text-slate-400 mt-1">
                      {permissionStatus === "granted" ? "Browser has authorized camera hardware access." : permissionStatus === "denied" ? "Blocked in browser permissions. Click lock icon in URL bar." : "Browser has not yet received user authorization."}
                    </p>
                  </div>
    ...
    /* Video Stream & Resolution Table */
              ...
              <div className="rounded-2xl bg-slate-950/60 border border-slate-800 p-4">
                ...
                {mediaStream?.getVideoTracks()[0]?.label && <div className="mt-3 p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs">
    ...
    /* TAB 2: WHY IS THE VIDEO DARK? TROUBLESHOOTER */
          ...
          {activeTab === "troubleshoot" && <div className="space-y-4">
    ...
    /* Step 1: Permission */
                ...
                <div className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 flex items-start space-x-3 text-xs">
                  {permissionStatus === "granted" ? <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" /> : <XCircle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" />}
                  <div className="flex-1">
                    ...
                    <p className="text-slate-400 text-[11px] mt-0.5">
                      {permissionStatus === "granted" ? "Passed: The browser allows this site to access camera hardware." : 'Blocked: Click the lock or camera icon next to the address bar and change Camera to "Allow".'}
                    </p>
                  </div>
                </div>
    ...
    /* Step 2: Physical Shutter / Lens Cover */
                ...
                <div className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 flex items-start space-x-3 text-xs">
                  {isLuminanceDark ? <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" /> : <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />}
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-slate-400 text-[11px] mt-0.5">
                      {isLuminanceDark ? "The camera stream is receiving 0% light. Most modern laptops (Lenovo ThinkPad, HP Spectre, Dell XPS, Asus) and webcams have a physical mechanical slider or red dot shutter over the lens." : "Passed: Sensor is receiving ambient light."}
                    </p>
                  </div>
                </div>
    ...
    /* Step 3: Track Muted by Operating System */
                ...
                <div className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 flex items-start space-x-3 text-xs">
                  {trackMuted ? <XCircle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" /> : <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />}
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-slate-400 text-[11px] mt-0.5">
                      {trackMuted ? "The operating system reports the video track is muted. Look for a camera key (often F8, F10, or a side switch) on your laptop keyboard." : "Passed: MediaStreamTrack is receiving frames from device driver."}
                    </p>
                  </div>
                </div>
    ...
    /* Step 4: Resolution & Pipeline Buffer */
                ...
                <div className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 flex items-start space-x-3 text-xs">
                  {reportedWidth > 0 && reportedHeight > 0 ? <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" /> : <XCircle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" />}
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`font-mono text-[11px] font-bold ${reportedWidth > 0 ? "text-emerald-400" : "text-rose-400"}`}>
                        {reportedWidth > 0 ? `${reportedWidth}\xD7${reportedHeight}` : "0\xD70"}
                      </span>
                    </div>
                    <p className="text-slate-400 text-[11px] mt-0.5">
                      {reportedWidth > 0 ? `Passed: Hardware stream is rendering at ${reportedWidth}\xD7${reportedHeight} resolution.` : 'Failed: No frame resolution received. Click "Restart Camera Hardware" below.'}
                    </p>
                  </div>
                </div>
    ...
    /* Step 5: Hardware Mutex / In Use by Another App */
                ...
                <div className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 flex items-start space-x-3 text-xs">
                  {cameraError?.type === "in_use" ? <XCircle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" /> : <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />}
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-slate-400 text-[11px] mt-0.5">
                      {cameraError?.type === "in_use" ? "Another program (Zoom, Teams, OBS, FaceTime) has an exclusive lock on your camera. Close those applications and retry." : "Passed: Camera hardware is not locked by another process."}
                    </p>
                  </div>
                </div>
              </div>
    ...
    /* TAB 3: RAW DIAGNOSTIC JSON */
          ...
          {activeTab === "raw" && <div className="space-y-3">
              <div className="flex items-center justify-between">
