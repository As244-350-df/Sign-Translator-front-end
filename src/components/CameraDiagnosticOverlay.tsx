...
export interface CameraDiagnosticOverlayProps {
  ...
  // Aliases for compatibility with different callers
  hardwarePermissionStatus?: 'checking' | 'prompt' | 'granted' | 'denied' | 'unsupported';
  videoRef?: React.RefObject<HTMLVideoElement | null>;
  cameraFacing?: 'user' | 'environment';
  activeResolution?: { width: number; height: number } | null;
  onToggleFacing?: () => void;
}
...
export const CameraDiagnosticOverlay: React.FC<CameraDiagnosticOverlayProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'why_not_working' | 'metrics' | 'troubleshoot' | 'raw'>('why_not_working');
  ...
  // Direct manual camera test state
  const [manualTestStatus, setManualTestStatus] = useState<'idle' | 'testing' | 'success' | 'error'>('idle');
  const [manualTestMessage, setManualTestMessage] = useState<string>('');
  const [manualTestStream, setManualTestStream] = useState<MediaStream | null>(null);
  const manualVideoRef = useRef<HTMLVideoElement | null>(null);

  const samplingCanvasRef = useRef<HTMLCanvasElement | null>(null);

  // Check iframe & enumerate hardware devices whenever overlay opens
  useEffect(() => {
    if (!isOpen) return;
    ...
    // If an error exists, default tab to why_not_working
  ...
  // Clean up manual test stream when overlay closes
  useEffect(() => {
  ...
  // Direct Manual Camera Test Handler
