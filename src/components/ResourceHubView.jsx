...
const ResourceHubView = ({
  ...
  const filteredSigns = allSignsList.filter(
    (s) => s.name.toLowerCase().includes(searchQuery.toLowerCase()) || s.description.toLowerCase().includes(searchQuery.toLowerCase()) || s.tags.some((t) => t.toLowerCase().includes(searchQuery.toLowerCase()))
  );
  ...
  />
    ...
    /* Category Tabs */
          ...
          <div className="flex items-center space-x-1 p-1 bg-slate-100 dark:bg-slate-900 rounded-xl w-full md:w-auto overflow-x-auto">
            {["all", "dictionary", "tutorials", "fingerspelling", "grammar"].map((cat) => <button
    key={cat}
    onClick={() => setSelectedCategory(cat)}
    className={`px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition-all whitespace-nowrap ${selectedCategory === cat ? "bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs" : "text-slate-500 hover:text-slate-900 dark:text-slate-400"}`}
  >
                {cat}
              </button>)}
          </div>
    ...
    /* Quick Searchable Dictionary Carousel / Grid */
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {filteredSigns.slice(0, 12).map((sign) => <button
    key={sign.id}
    onClick={() => {
  ...
  >
    ...
    /* Dictionary Detail Modal popup if active */
      ...
      {activeDictionarySign && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
