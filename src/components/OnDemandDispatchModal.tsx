...
interface OnDemandDispatchModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConnectCall: (interpreterId: string) => void;
  settings: AppSettings;
}

export const OnDemandDispatchModal: React.FC<OnDemandDispatchModalProps> = ({
  isOpen,
  onClose,
  onConnectCall,
  settings,
}) => {
  const [selectedLanguage, setSelectedLanguage] = useState<SignLanguageCode>(settings.primarySignLanguage);
  const [selectedSpecialty, setSelectedSpecialty] = useState<string>('Medical & Healthcare');
  const [matchingState, setMatchingState] = useState<'idle' | 'searching' | 'found' | 'timeout'>('idle');
  const [countdown, setCountdown] = useState<number>(30);
  const [matchedInterpreter, setMatchedInterpreter] = useState<Interpreter | null>(null);
  ...
  // Countdown timer when found (accept within 30s)
  useEffect(() => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-lg w-full shadow-2xl relative p-6 sm:p-8">
          ...
          <div className="mt-6 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setSelectedLanguage(e.target.value as SignLanguageCode)}
        ...
        {matchingState === 'searching' && (
          <div className="mt-8 text-center space-y-4 py-8">
        ...
        {matchingState === 'found' && matchedInterpreter && (
          <div className="mt-6 space-y-4">
