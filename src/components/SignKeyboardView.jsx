...
import { SIGN_ALPHABET, COMMON_SIGNS } from "../data/mockData";
...
const SignKeyboardView = ({
  settings,
  onOpenTutorial
}) => {
  const [selectedKey, setSelectedKey] = useState(SIGN_ALPHABET[0]);
  const [composedSentence, setComposedSentence] = useState("HELLO");
  ...
  const handleKeyPress = (item) => {
    ...
    if (settings.soundEffects) {
      speakText(item.name, void 0, 1.2, 1.1);
    }
  ...
  const handleSpeak = () => {
    speakText(composedSentence, void 0, settings.speechVoiceRate, settings.speechVoicePitch);
  };
  const filterByFinger = (items) => {
  ...
  return <div className="space-y-6">
    ...
    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeCategory === "alphabet" ? "bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
  >
    ...
    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeCategory === "numbers" ? "bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
  >
            0-9 Numbers (10)
          </button>
          <button
    ...
    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeCategory === "words" ? "bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
  >
    ...
    className={`px-3 py-2 rounded-xl text-xs font-bold flex items-center space-x-1.5 transition-all ${practiceMode ? "bg-amber-500 text-white shadow-xs" : "bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-200"}`}
  >
  ...
  >
    ...
    className={`px-2.5 py-1 rounded-xl text-[11px] font-bold transition-all ${activeFingerFilter === "all" ? "bg-indigo-600 text-white shadow-xs" : "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200"}`}
  >
                All Signs
              </button>

              {["thumb", "index", "middle", "ring", "pinky"].map((fKey) => {
    const meta = FINGER_METADATA[fKey];
    const isActive = activeFingerFilter === fKey;
    return <button
      key={fKey}
      onClick={() => setActiveFingerFilter(fKey)}
      className={`px-2.5 py-1 rounded-xl text-[11px] font-bold transition-all flex items-center space-x-1 ${isActive ? `bg-gradient-to-r ${meta.color} text-white shadow-xs scale-105` : "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200"}`}
    >
                    <span>{meta.icon}</span>
                    <span>{meta.name}</span>
                  </button>;
    ...
    >
      ...
      /* Finger Activation Micro-Badge Dots */
                      ...
                      <div className="flex items-center space-x-0.5 mt-1.5">
                        {["thumb", "index", "middle", "ring", "pinky"].map((fk) => {
      const isEngaged = prof.primaryFingers.includes(fk);
      const meta = FINGER_METADATA[fk];
      return <span
        key={fk}
        title={`${meta.name}: ${isEngaged ? "Active" : "Curled"}`}
        className={`w-1.5 h-1.5 rounded-full transition-all ${isEngaged ? `bg-gradient-to-tr ${meta.color} ring-1 ring-white dark:ring-slate-900 scale-125` : "bg-slate-300 dark:bg-slate-700 opacity-40"}`}
      />;
    })}
                      </div>
    ...
    >
      ...
      return <span
        key={fk}
        className={`w-1.5 h-1.5 rounded-full transition-all ${isEngaged ? `bg-gradient-to-tr ${meta.color} scale-125` : "bg-slate-300 dark:bg-slate-700 opacity-40"}`}
      />;
    })}
                      </div>
    ...
    >
      ...
      return <span
        key={fk}
        className={`w-1.5 h-1.5 rounded-full ${isEngaged ? `bg-gradient-to-tr ${meta.color}` : "bg-slate-300 dark:bg-slate-700 opacity-40"}`}
      />;
    })}
                      </div>
